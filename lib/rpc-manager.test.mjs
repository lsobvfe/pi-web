...
test("RPC session startup opens an existing session file once and enforces the runtime cwd policy", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));
  ...
  assert.match(startupSource, /const sessionCwd = assertRuntimeWorkspaceCwd\(sessionManager\.getCwd\(\)\)/);
...
test("normal session teardown paths use graceful extension shutdown", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const lifecycleSource = await readFile(new URL("./session-lifecycle.ts", import.meta.url), "utf8");
  ...
  assert.match(lifecycleSource, /await getRpcSession\(sessionId\)\?\.shutdown\(\)/);
