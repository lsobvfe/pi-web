...
async function loadAllSessions(): Promise<SessionInfo[]> {
  ...
  attached.forEach((session, index) => {
    sessionCustomData.set(session, scanned[index]?.customData ?? {});
  });
  return attached;
}
...
export function buildSessionContext(
  ...
  for (const entry of sliced) {
    const m = sessionEntryToUiMessage(entry, options);
    if (m) {
      messages.push(m);
...
export const entryToUiMessage = sessionEntryToUiMessage;
