...
test("agent command HTTP rejections are distinguishable from transport failures", async (t) => {
  const rejectedRequest = async () => new Response(
    JSON.stringify({
  ...
  await assert.rejects(
    sendAgentCommand(rejectedRequest, "session-id", { type: "prompt", message: "hello" }),
    (error) => {
  ...
  await assert.rejects(
    sendAgentCommand(failedRequest, "session-id", { type: "prompt", message: "hello" }),
    (error) => {
