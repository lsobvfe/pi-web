...
export function useAgentSession(opts: UseAgentSessionOptions) {
  ...
  if (!eventConnectionRef.current) {
    eventConnectionRef.current = new AgentEventConnection({
      createSource: (sid) => piWebEventSource(`/api/agent/${encodeURIComponent(sid)}/events`),
  ...
  const loadSession = useCallback(async (sid: string, showLoading = false, includeState = false) => {
    ...
    try {
      ...
      const res = await piWebFetch(`/api/sessions/${encodeURIComponent(sid)}?${params}`);
      if (res.status === 404) {
        if (showLoading) {
          setData(null);
      ...
      try {
        const stateRes = await piWebFetch(`/api/sessions/${encodeURIComponent(sid)}/state`);
  ...
  const loadContext = useCallback(async (sid: string, leafId: string | null, before?: string | null, options?: { tail?: number; signal?: AbortSignal }) => {
    try {
      ...
      const res = await piWebFetch(url, { signal: options?.signal });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
  ...
  const ensureNewSession = useCallback(async () => {
    ...
    const promise = (async () => {
      ...
      const res = await piWebFetch("/api/agent/new", {
        method: "POST",
  ...
  const scheduleEventStreamClose = useCallback((sid: string) => {
    ...
    const checkServerIdle = async () => {
      ...
      try {
        const res = await piWebFetch(`/api/agent/${encodeURIComponent(sid)}`);
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
    ...
    switch (event.type) {
      ...
      case "agent_end":
        ...
        if (sessionIdRef.current) {
          ...
          piWebFetch(`/api/agent/${encodeURIComponent(sessionIdRef.current)}`)
            .then((r) => r.json())
  ...
  const loadModels = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      const res = await piWebFetch(modelsUrl, signal ? { signal } : undefined);
      if (!res.ok) {
        let detail = "";
        try {
